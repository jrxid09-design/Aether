...
function writeEnv() {
    ...
    fs.writeFileSync(path.join(DIR, ".env"),
        `UPLOAD_LOCATION=${dockerPath(path.join(root, "library"))}\n` +
...
async function status() {
    ...
    try {
        ...
        return { available: true, poolSet, installed: services.length > 0, running, services, url: URL, dataRoot: root };
    }
    catch {
        return { available: false, poolSet, installed: false, running: 0, services: [], reason: "Docker tidak tersedia atau belum berjalan.", url: URL, dataRoot: root };
    }
}
...
module.exports = { status, up, down, dataRoot };
