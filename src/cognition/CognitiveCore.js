...
class CognitiveCore {
    ...
    async encodeExperienceIfSignificant({ at, before, after, appraisal, eventType }) {
        ...
        const factors = {
            ...
            commitmentImpact:
                startsWithAny(appraisal?.eventId, ["COMMITMENT_"]) ? 1 : 0,
        ...
        await this.continuity.store.upsertExperience(
            experienceId, significance, encoded);

        this.continuity.state.autobiography.significantCount += 1;
        this.continuity.state.autobiography.recent =
            [...this.continuity.state.autobiography.recent, {
                experienceId, significance, at
            }].slice(-this.config.experience.recentBufferSize);
    ...
    async openPrediction(spec) {
        ...
        await this.continuity.store.upsertPrediction(
            prediction.predictionId, prediction.status, prediction);
    ...
    async resolvePrediction(predictionId, correct) {
        ...
        const out = await this.continuity.feed(makeEnvelope({
            type, source: "acc.prediction", provenance: "SYSTEM_EVENT",
            payload: { predictionId }, clock: this.clock
        }));

        const status = correct
            ? "RESOLVED_CORRECT" : "RESOLVED_INCORRECT";
        await this.continuity.store.upsertPrediction(predictionId, status, open);
...
let lastType = null;   // konteks event saat encoding pengalaman
