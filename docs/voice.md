# Aether Hidup — Avatar & Suara

Layar **Aether** di Console adalah wajah entitasnya: minibot yang
...
tapi kehadirannya membuat Aether terasa hadir, bukan sekadar kotak teks.

## Suara keluar (TTS)

Aether berbicara memakai suara OS lewat `speechSynthesis`:
**offline, tanpa server**. Pilih suara di pojok kanan atas layar
Aether; kalau ada suara berbahasa Indonesia terpasang di Windows, itu
...
STT **konfigurabel & opsional**. Tanpa `AETHER_STT_URL`, tombol mic
dinonaktifkan dengan penjelasan — mengetik tetap bisa dan Aether tetap
...
- Percakapan di layar Aether berbagi runtime yang sama dengan Chat/CLI,
  termasuk injeksi memori otomatis — jawaban suaranya pun sudah
  mempertimbangkan apa yang Aether ingat.
...
- STT diverifikasi sampai lapisan endpoint + fallback; transkripsi
  nyata perlu `AETHER_STT_URL` aktif dan mic asli.
