/* Aether Voice Widget — dibuat oleh Aether, 2026-08-17
   Mendengarkan feed.json dari server suara Aether (port 8643)
   dan memutar suara aslinya langsung di Console. */
(function () {
  if (window.__aetherVoice) return; window.__aetherVoice = true;
  var FEED = 'http://127.0.0.1:8643/voice/feed.json';
  var lastTs = 0, playing = false, muted = (localStorage.getItem('aether-voice-mute') === '1');

  var w = document.createElement('div');
  w.id = 'aether-voice-widget';
  ...
  w.innerHTML = '<span id="av-eq" style="display:inline-flex;gap:2px;align-items:flex-end;height:12px">' +
    ...
    '<span id="av-label">\u266A Aether</span>';
  document.body.appendChild(w);
  var label = w.querySelector('#av-label'), eq = w.querySelector('#av-eq');

  w.addEventListener('click', function () {
    muted = !muted;
    localStorage.setItem('aether-voice-mute', muted ? '1' : '0');
    label.textContent = muted ? '\u266A Aether (bisu)' : '\u266A Aether';
  });
  ...
  window.aetherVoiceSay = function (t) { label.textContent = t || '\u266A Aether'; };
})();
